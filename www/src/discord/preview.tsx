    ...
    >
      <img
        className={`Voice_avatar__htiqH ${
          speaking ? " Voice_avatarSpeaking__Lali" : ""
        }`}
        src={`${avatar}#userIDtrick/${user.id}/`}
        alt=""
      />
      <div className="Voice_user__8fGwX">
        <span className="Voice_name__TALd9" style={style_span}>
          {user.username}
        </span>
      </div>
    </li>
...
export default function Preview({ users, style }: PreviewProps) {
  ...
  useEffect(() => {
    setSortedUsers(
      [...users].sort((a, b) =>
        a.username.toLocaleLowerCase() < b.username.toLocaleLowerCase() ? -1 : 1
      )
    );
  }, [users]);
  ...
  return (
    <>
      ...
      <div id="discord-preview">
        <div
          style={{
            fontFamily: "Whitney, sans-serif, sans",
            backgroundColor: "transparent",
          }}
        >
          <div className="Voice_voiceContainer__adk9M">
            <ul className="Voice_voiceStates__a121W">
              {sortedUsers.map((user: DiscordUser, index: number) => (
                <PreviewSpeaker key={index} user={user} />
              ))}
            </ul>
          </div>
        </div>
